...
void DetectCellsOverTemp(void)
{
	...
	if(WARNING_SECOND_LEVEL == g_SystemWarning.COT)
	{
		...
		{
			otErrCnt++;
			if (otErrCnt > CELL_COT_FAULT_DLY) // 持续1S
			{
				otErrCnt = 0;
	...
	if(WARNING_SECOND_LEVEL == g_SystemWarning.DOT)
	{
		...
		{ 
			otErrCnt++;
			if (otErrCnt > CELL_DOT_FAULT_DLY) // 持续1S
			{
				otErrCnt = 0;
	...
	else if(DISCHARGE == g_BatteryMode)
	{
		...
		{
			if(otErrCnt > CELL_DOT_WARNING_DLY) // 持续1S
			{
...
void DetectCellsUnderTemp(void)
{
	...
	if(WARNING_SECOND_LEVEL == g_SystemWarning.CUT)
	{
		...
		{       
			if(utErrCnt > CELL_CUT_FAULT_DLY) // 持续1S
			{
				utErrCnt = 0;
	...
	if(WARNING_SECOND_LEVEL == g_SystemWarning.DUT)
	{
		...
		{
			if(utErrCnt > CELL_DUT_FAULT_DLY) // 持续1S
			{
				utErrCnt = 0;
	...
	if(CHARGE == g_BatteryMode)
	{
		...
		{
			if(utErrCnt > CELL_CUT_WARNING_DLY) // 持续1S
			{
				utErrCnt= 0;
...
void DetectPackUv(void)
{
	...
	{
		if (pUvErrCnt > PACK_UV_WARNING_DLY) // 持续5S
		{
			pUvErrCnt = 0;
